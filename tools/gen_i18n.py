...
# A coordinator's BRAND on the glass, and the only two keys allowed to carry
# one. The pairing screen offers a choice between two apps and names both,
# which is the one screen where a brand is the answer to the question being
# asked. Anywhere else it is a guess: the pick is a file-static reset on entry,
# so no screen outside pairing knows which coordinator an owner runs, and four
# signing strings told a BlueWallet owner to open Sparrow -- "open in Sparrow",
# the QR and SD handoff captions, and the empty card lane. They say "your
# coordinator" now, which is the word the rest of the product uses and the one
# the readme's table defines.
#
# EVERY LOCALE, not English alone. Those four were translated into twenty
# languages with the brand carried through untouched, so a rule reading
# i18n/en.json would have reported the glass clean while twenty locales still
# named one app. A third coordinator joins this list the day anything names it.
APP_NAME_OK = {"I_APP_DESKTOP", "I_APP_MOBILE"}
APP_NAME_RE = re.compile(r"Sparrow|BlueWallet")
...
UNTRANSLATED_OK = {
    ...
    # Born under the English-only rule; English in the other twenty
    # until the sweep.
    "L_BACKUP_KEF_OPENED", "L_BACKUP_WORDS_USED",
    ...
    # The signing handoff, which stopped naming one coordinator by brand, and
...
def main():
    ...
    for stem, ident, fc, _nat in LOCALES:
        ...
        for k in keys:
            ...
            brand = APP_NAME_RE.search(v)
            if brand and k not in APP_NAME_OK:
                errors.append(f"{stem}:{k}: names a coordinator brand "
                              f"({brand.group(0)}), and only "
                              f"{' and '.join(sorted(APP_NAME_OK))} may. "
                              f"Every other screen says 'your coordinator', "
                              f"because nothing outside pairing knows which "
                              f"one this owner runs")
            if fc == "lat":
