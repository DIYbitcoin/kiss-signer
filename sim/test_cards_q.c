...
int test_cards_q(void)
{
    ...
    ok("11 identical -> SAME",
       q.verdict == WC_Q_SAME && wallet_cards_blocked(&q) &&
       q.distinct == 1 && q.dups == 10 && q.period == 1 &&
       (q.flags & WC_F_SAME) && (q.flags & WC_F_PERIOD) &&
       !(q.flags & WC_F_SORTED));
    wallet_cards_judge(w, 23, &q);
    ok("23 identical -> SAME", q.verdict == WC_Q_SAME && wallet_cards_blocked(&q));
    ...
    ok("5 word block, typed over -> PERIOD",
       q.verdict == WC_Q_PERIOD && wallet_cards_blocked(&q) && q.period == 5);
    ...
    ok("smallest repeat 6 at n=11 -> not periodic",
       q.period == 0 && !(q.flags & WC_F_PERIOD) && !wallet_cards_blocked(&q));
    ...
    ok("ascending, wide gaps -> SORTED",
       q.verdict == WC_Q_SORTED && !wallet_cards_blocked(&q) &&
    ...
    wallet_cards_judge(w, 10, &q);
    ok("10 words -> SHORT even all identical",
       q.verdict == WC_Q_SHORT && !wallet_cards_blocked(&q));
    wallet_cards_judge(NULL, 11, &q);
