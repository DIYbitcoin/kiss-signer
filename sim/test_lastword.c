...
static int word_index(const char *w)
{
    for (int i = 0; i < 2048; i++) {
        const char *c = kiss_lastword_word((uint16_t)i);
...
static int completes(const char *prefix, uint16_t idx)
{
    ...
    snprintf(m, sizeof m, "%s %s", prefix, kiss_lastword_word(idx));
    return kiss_seed_validate(m) == 0;
}
...
int test_lastword(void)
{
    ...
    // ---- wordlist access ----
    ok("word 0 is abandon", kiss_lastword_word(0) &&
       strcmp(kiss_lastword_word(0), "abandon") == 0);
    ok("word 2047 is zoo", kiss_lastword_word(2047) &&
       strcmp(kiss_lastword_word(2047), "zoo") == 0);
    ok("word 2048 is NULL", kiss_lastword_word(2048) == NULL);

    // ---- canonical zero entropy vectors ----
    abandon_str(prefix, sizeof prefix, 11);
    int n = kiss_lastword_candidates(prefix, cand);
    ...
    n = kiss_lastword_candidates(prefix, cand);
    ...
    // ---- round trip: a full mnemonic's own last word is in the set ----
    {
        ...
        ok("from_entropy 16 rc", kiss_seed_from_entropy(e16, 16, words, sizeof words) == 0);
        char *last = strrchr(words, ' ');
        *last = '\0';
        n = kiss_lastword_candidates(words, cand);
        ok("16 byte round trip: 128 candidates", n == 128);
        ok("16 byte round trip: own last word present",
           set_has(cand, n, word_index(last + 1)));

        ok("from_entropy 32 rc", kiss_seed_from_entropy(e32, 32, words, sizeof words) == 0);
        last = strrchr(words, ' ');
        *last = '\0';
        n = kiss_lastword_candidates(words, cand);
    ...
    // kiss_lastword_index binary searches, which is only correct while the
    ...
    // and word_index above stays a linear scan so the two never share a bug.
    {
        ...
        for (int i = 0; i < 2048; i++) {
            const char *c = kiss_lastword_word((uint16_t)i);
            if (c && kiss_lastword_index(c) == i) round++;
            if (i && strcmp(c, kiss_lastword_word((uint16_t)(i - 1))) <= 0)
                ascending = 0;
        }
        ok("index: round trips for all 2048 words", round == 2048);
        ok("index: the list is strictly ascending (the search's premise)", ascending);
    }
    ok("index: off list word -> -1", kiss_lastword_index("zzzz") == -1);
    ok("index: NULL -> -1", kiss_lastword_index(NULL) == -1);
    ok("index: empty -> -1", kiss_lastword_index("") == -1);

    // ---- rejections ----
    ok("NULL -> -1", kiss_lastword_candidates(NULL, cand) == -1);
    abandon_str(prefix, sizeof prefix, 10);
    ok("10 words -> -1", kiss_lastword_candidates(prefix, cand) == -1);
    abandon_str(prefix, sizeof prefix, 12);
    ok("12 words -> -1", kiss_lastword_candidates(prefix, cand) == -1);
    abandon_str(prefix, sizeof prefix, 22);
    ok("22 words -> -1", kiss_lastword_candidates(prefix, cand) == -1);
    abandon_str(prefix, sizeof prefix, 24);
    ok("24 words -> -1", kiss_lastword_candidates(prefix, cand) == -1);
    abandon_str(prefix, sizeof prefix, 10);
    strncat(prefix, " zzzz", sizeof prefix - strlen(prefix) - 1);
    ok("off list word -> 0", kiss_lastword_candidates(prefix, cand) == 0);
