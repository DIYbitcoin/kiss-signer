...
/* KISS: the ONE line of this vendored tree that is not upstream.
 *
 * Every sha2_256_update() and sha2_256_final_len() below calls
 * sha2_256_compress(), so that symbol is the single seam a hardware SHA can be
 * dropped into -- there is no higher one.  With PQ_SHA256_COMPRESS_HOOK defined
 * this definition is renamed and pq_hw_sha.c supplies the dispatcher under the
 * old name; the call sites are untouched, so they reach the accelerator.
 * Undefine it and this file is upstream again, byte for byte.  See UPSTREAM.md.
 */
