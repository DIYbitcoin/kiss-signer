...
**ECDSA (purpose 44 / 49 / 84).** Follows the
[BIP461 draft](https://github.com/bitcoin/bips/blob/master/bip-0461.md)
reviewed 2026-09-17: RFC6979 HMAC-SHA256 with the message hash reduced modulo
the curve order for nonce derivation. The first attempt has no extra data.
High R retries use a four-byte little-endian counter starting at 1 followed
by 28 zero bytes, until R < 2^255. Invalid nonce/r/s retries advance the
RFC6979 generator separately. S is normalized to at most half the curve order.
This is `EC_FLAG_GRIND_R` in libwally and matches Bitcoin Core. Sighash is the standard
...
The local libwally patch verifies each ECDSA signature before returning it,
clears output on verification failure, and refuses grind-counter overflow.
This catches computation faults; malicious firmware can bypass it. Independent
comparison of real signatures before broadcast remains the check for malice.
The boot self-test is separate and does not replace per-signature verification.

`sim/bip461_vectors.h` pins 13 independent KISS fixtures, including hashes at
and above the curve order, boundary valid keys, no grinding and multiple grind
rounds. These are generated by `tools/sign_fixtures/gen_bip461_vectors.py`,
not published BIP test vectors. `kisstest` checks exact bytes, validity, low R,
the 70-byte DER bound and invalid keys. Existing PSBT and boot fixtures remain
unchanged. BIP461 is still a draft; future revisions require review. It does
not change the Schnorr rule below.
