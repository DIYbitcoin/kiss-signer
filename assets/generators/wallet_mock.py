...
def render(active, status="caution", fp=None, grid_a=44, bake_chip=True,
    ...
    # tiles (labels stay baked: animating them made the unlock jank on device)
    tw = 160
...
# the baked CAUTION never changed, which made it a fake status light)
...
# ---- ambient ₿ mote sprite (main/mote_img.c/.h) ----
# U+20BF is missing from every licensable system font here, so build it the way
# the symbol is constructed anyway: the Arial Rounded "B" plus two vertical
# strokes that pass through it and protrude top + bottom. Heavily supersampled;
# stroke width matches the B's own stem so it reads as ONE glyph, not B + dots.
S = 8
bw, bh = 22 * S, 30 * S
...
stroke_w = 1.0 * S                               # ~2px at final scale = stem weight
for fx in (0.38, 0.64):                          # short nubs: 2px out, 2px into the ink
    x = bb[0] + (bb[2] - bb[0]) * fx
    bd.rectangle([x - stroke_w, bb[1] - 2 * S, x + stroke_w, bb[1] + 2 * S], fill=INK)
    bd.rectangle([x - stroke_w, bb[3] - 2 * S, x + stroke_w, bb[3] + 2 * S], fill=INK)
bim = bim.crop(bim.getbbox())                    # tight to the glyph
bim = bim.resize((max(1, bim.width // S), max(1, bim.height // S)), Image.LANCZOS)
...
data = np.concatenate([
    ...
    al.reshape(-1)])
with open(OUT + "/mote_img.c", "w") as f:
    f.write("// GENERATED by assets/generators/wallet_mock.py - do not hand-edit\n")
    f.write('#include "lvgl.h"\n\n')
    f.write("static const uint8_t mote_btc_map[] = {%s};\n"
...
with open(OUT + "/mote_img.h", "w") as f:
    f.write("// GENERATED by assets/generators/wallet_mock.py - do not hand-edit\n"
            "#pragma once\n#include \"lvgl.h\"\n\n"
            "extern const lv_image_dsc_t img_mote_btc;   // ambient home ₿ mote\n")
print("wrote main/mote_img.c/.h (img_mote_btc %dx%d) + /tmp/mote_btc_mock.png" % bim.size)
