...
// wallet derives from the seed the user stored (kiss_seed.c). Steps 3-6
...
int kiss_entropy_mix(const uint8_t a[32], const uint8_t b[32], uint8_t out[32])
{
    if (!a || !b || !out)
        return -1;
    uint8_t cat[64];
...
// Chained rather than accumulated into one buffer, exactly as kiss_tapent.c
...
// kiss_duress.h), and the UI keys off it: the screen that configures the
...
void kiss_session_discard_prepared(void)
{
...
int kiss_session_open(const char *passphrase)
{
    ...
    if (rc == 0)
        rc = kiss_session_activate_prepared();
    return rc;
}
...
int kiss_script(void) { return s_script; }
...
int kiss_session_sp_address(char *out, size_t out_len)
{
    if (!s_session)
        return 1;
...
int kiss_session_sp_scan_export(char *out, size_t out_len)
{
    if (!s_session)
        return 1;
...
int kiss_session_descriptor(char *out, size_t out_len)
{
    if (!s_session)
        return 1;
    struct ext_key acct;
...
int kiss_session_bw_export(char *out, size_t out_len)
{
    if (!s_session)
        return 1;
    struct ext_key acct;
...
int kiss_selftest(uint8_t out_fingerprint[4])
{
    if (out_fingerprint)
...
// Unlike kiss_selftest above, this one is NOT compiled out in release. It
...
// -1 = not run yet. Cached because kiss_psbt_sign consults it on every sign
// and the answer cannot change within a boot.
static int s_sign_selftest = -1;

#ifndef KISS_RELEASE
static int s_sign_force_fail;
void kiss_sign_selftest_force_fail(int on)
{
