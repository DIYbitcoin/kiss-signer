...
// look finished. Mirrors kiss_dice_q: pure, no UI, host-testable.
//
// Deliberately a separate file from kiss_lastword.c: that one pulls in
...
// kiss_dice_q.c beside kiss_dice.c.
//
...
// kiss_lastword_candidates() must keep answering 128 for it forever — and it
...
// kiss_cards_q.c. Sizes between 11 and 23 cannot be reached from the UI and
...
void kiss_cards_judge(const uint16_t *idx, unsigned n, kiss_cards_q_t *out);
...
int  kiss_cards_blocked(const kiss_cards_q_t *q);
