// See kiss_duress.h. Two halves, both deliberately dependency-light:
//
...
// ---- unlock routing ------------------------------------------------------
// See kiss_duress.h for why this ignores the configured stroke, and why it
...
//
// Routed through kiss_duress_route rather than repeating its body, so there
...
// ---- the word ------------------------------------------------------------
//
// Validation and matching sit ABOVE the storage split, like kiss_duress_route
...
int kiss_duress_word_len(void) { return kiss_duress_word_get(NULL); }
...
// Deliberately NOT in kiss_seed.c's KEEP_KEYS: erasing the wallet must take
...
int kiss_duress_real(void) { return read_u8(K_REAL); }
...
void kiss_duress_forget(void)
{
    ...
    (void)kiss_duress_word_set(NULL, 0);
}
...
int kiss_duress_real(void) { return s_real; }
